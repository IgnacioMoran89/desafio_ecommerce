...
RSpec.describe Category, type: :model do
  it 'can create categories' do
    category = Category.create(name: 'Videojuegos')
    expect(category).to be_instance_of(Category)
  end

  it 'cannot create categories with same name' do
    category1 = Category.create(name: 'Videojuegos')
    category2 = Category.create(name: 'Videojuegos')
    expect(category2).to_not be true
  end

  it 'cannot create categories with blank name' do
